...
# 네이버 지도에서 사람이 보는 결과와 최대한 가까워지도록 시군구 검색망을 확대한다.
# 기존 목록에 빠져 있던 인천 계양/검단권, 경기 외곽, 지방 시군을 보강한다.
EXTRA_AREAS = [
...
def options(_): return ("", 204)

@app.get("/api/health")
def health(): return jsonify({"ok": True, "service": "brand-review-railway", "version": "1.4", "search_areas": len(SEARCH_AREAS)})

@app.get("/api/search-plan")
def search_plan(): return jsonify({"ok": True, "areas": SEARCH_AREAS, "count": len(SEARCH_AREAS)})

@app.post("/api/search-area")
def search_area():
    body = request.get_json(silent=True) or {}
    brand, area = str(body.get("brand", "")).strip(), str(body.get("area", "")).strip()
    if len(brand) < 2 or not area: return jsonify({"ok": False, "error": "브랜드명과 검색지역이 필요합니다"}), 400
    try:
    ...
    except Exception as exc: return jsonify({"ok": False, "error": f"{type(exc).__name__}: {exc}"}), 502

@app.post("/api/search")
def search_brand():
    body = request.get_json(silent=True) or {}; brand = str(body.get("brand", "")).strip()
    if len(brand) < 2: return jsonify({"ok": False, "error": "브랜드명을 2글자 이상 입력하세요"}), 400
    try:
        crawler = NaverPlaceCrawler(timeout=18, pause=0.06); stores = discover_stores(crawler, brand)
...
def check_store():
    body = request.get_json(silent=True) or {}
    name, address, place_id = str(body.get("name", "")).strip(), str(body.get("address", "")).strip(), str(body.get("place_id", "")).strip()
    place_type = str(body.get("place_type", "restaurant") or "restaurant").strip(); days = max(1, min(30, int(body.get("days", 7) or 7)))
    if len(name) < 2: return jsonify({"ok": False, "error": "매장명이 필요합니다"}), 400
    try:
        crawler = NaverPlaceCrawler(timeout=18, pause=0.10)
        if place_id.isdigit():
            try: reviews, review_url = crawler._graphql_reviews(place_id, place_type, 50)
            except Exception: reviews, review_url = crawler._apollo_reviews(place_id, place_type, 50)
            resolved_place_id = place_id
        else:
            match, reviews, review_url = crawler.fetch_latest_reviews(name, address, limit=50); resolved_place_id = match.place_id
        rows = recent([{"id": r.review_id, "text": r.text, "created_at": r.created_at, "rating": r.rating} for r in reviews], days)
        analyzed = []
        for row in rows:
            a = analyze_review(row.get("text", "")); analyzed.append({**row, "bad_hits": a.bad_hits, "good_hits": a.good_hits, "score": a.score, "level": a.level})
...
@app.get("/")
def dashboard(): return send_from_directory(os.path.join(os.path.dirname(__file__), "..", "railway_site"), "index.html")
